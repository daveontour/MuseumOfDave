...
// AdditionalAttachment represents extra attachments (photos/videos/files beyond the first)
...
// FindAttachmentFile locates an attachment file by URI. Tries conversation dir first,
// then subdirs (photos/, videos/, files/), then export root if provided.
func FindAttachmentFile(conversationDir, uri, exportRoot string) (string, bool) {
	if uri == "" {
		return "", false
	}

	// Try relative to conversation directory
	relPath := filepath.Join(conversationDir, uri)
	if pathExists(relPath, false) {
		return relPath, true
	}

	// Try filename only in conversation dir
	...
	// Try subdirs: photos/, videos/, files/
	...
	// Search for file by filename in conversation dir and subdirs
...
// GetFirstAttachment extracts the primary attachment from a message (photos > videos > files).
// Returns (filename, mimeType, data, additionalAttachments).
func GetFirstAttachment(msg FacebookMessage, conversationDir, exportRoot string) (string, string, []byte, []AdditionalAttachment) {
	...
	// Try .heic -> .jpg fallback for photos
	...
	// Process files if no photo or video
	...
	// If we have photo, add videos and files as additional
	if primaryData != nil && len(photos) > 0 {
