...
// LocationStorage handles location storage operations
type LocationStorage struct {
	db *DB
}

// NewLocationStorage creates a new location storage instance
func NewLocationStorage(db *DB) *LocationStorage {
	return &LocationStorage{db: db}
}

// SaveOrUpdateLocation saves or updates a location. Deduplication by name + coordinates.
// If lat/long provided: match by name and coordinates within tolerance.
// If no coordinates: match by name where lat/long are NULL.
// Returns (created bool, error)
func (s *LocationStorage) SaveOrUpdateLocation(ctx context.Context, name, address string, latitude, longitude *float64, source, sourceRef string) (created bool, err error) {
	...
	if latitude != nil && longitude != nil {
		// Find existing by name and coordinates within tolerance
		...
		// Find existing by name with no coordinates
		err = tx.QueryRow(ctx, `SELECT id FROM locations WHERE name = $1
...
func (s *LocationStorage) UpdateLocationRegions(ctx context.Context) error {
	_, err := s.db.Pool.Exec(ctx, "SELECT update_location_regions()")
	if err != nil {
