...
// ClearFacebookAllData removes all data imported by the facebook, facebook-albums,
// facebook-places, and facebook-posts commands. Runs in a single transaction so
// either all tables are cleared or none are (on error).
func (db *DB) ClearFacebookAllData(ctx context.Context) error {
	...
	_, err = tx.Exec(ctx, `
		...
		DELETE FROM media_blob WHERE id IN (SELECT media_blob_id FROM deleted_items)
	`)
	if err != nil {
