...
func NewFacebookPostStorage(db *DB) *FacebookPostStorage {
	return &FacebookPostStorage{db: db}
}

// FindPostByTimestampAndTitle looks up a post by its Unix timestamp and title.
// Returns (post_id, true) if found, (0, false) otherwise.
...
// Returns (post_id, was_created, error).
...
// Returns the number of items imported, or an error if any insert fails.
