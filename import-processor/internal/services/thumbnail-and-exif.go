...
func (p *Processor) findImageMagickCommand() string {
	// Equivalent to your self.find_imagemagick_command()
	return "magick"
}
...
func (p *Processor) CreateThumbAndGetExif(
	...
	magickCmd := p.findImageMagickCommand()
	...
	if processThumbnail && processExif {
		// Python uses -write info:fd:2 to send JSON to stderr
		// We will capture Stdout for the image and Stderr for the JSON
		args := []string{
			"-",
			"-quiet",
			"-format", formatString,
